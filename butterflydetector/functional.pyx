...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef void scalar_square_add_2dconstant(float[:, :] field, float[:] x, float[:] y, float[:] width, float[:] height, float[:] v) nogil:
    cdef long minx, miny, maxx, maxy
    cdef Py_ssize_t i, xx, yy
    cdef float cx, cy, cv, cwidth, cheight
    ...
    for i in range(x.shape[0]):
        ...
        minx = (<long>clip(cx - cwidth, 0, field.shape[1] - 1))
        maxx = (<long>clip(cx + cwidth, minx + 1, field.shape[1]))
        miny = (<long>clip(cy - cheight, 0, field.shape[0] - 1))
        maxy = (<long>clip(cy + cheight, miny + 1, field.shape[0]))
        for xx in range(minx, maxx):
            for yy in range(miny, maxy):
                field[yy, xx] += cv
