...
from .utils import scalar_square_add_single, scalar_square_add_2dsingle
...
class Processor(object):
    ...
    def soft_nms(self, annotations):
        ...
        for ann in annotations:
            if not 'butterfly' in self.model.head_names[0]:
                joint_scales = (np.maximum(4.0, ann.joint_scales)
                                if ann.joint_scales is not None
                                else np.ones((ann.data.shape[0]),) * 4.0)
                zipped_res = zip(ann.data, occupied, joint_scales)
            else:
                joint_scales_w = (np.maximum(4.0, ann.joint_scales_w)
                                if ann.joint_scales_w is not None
                                else np.ones((ann.data.shape[0]),) * 4.0)
                joint_scales_h = (np.maximum(4.0, ann.joint_scales_h)
                                if ann.joint_scales_h is not None
                                else np.ones((ann.data.shape[0]),) * 4.0)
                zipped_res = zip(ann.data, occupied, joint_scales_w, joint_scales_h)
            ...
            for l in zipped_res:
                ...
                elif not 'butterfly' in self.model.head_names[0]:
                    scalar_square_add_single(occ, xyv[0], xyv[1], joint_s, 1)
                else:
                    scalar_square_add_2dsingle(occ, xyv[0], xyv[1], np.clip(xyv[3]/5,a_min=2, a_max=None), np.clip(xyv[4]/5,a_min=2, a_max=None), 1)
    ...
    def keypoint_sets_from_annotations(annotations):
        ...
        return keypoint_sets, bboxes, scores

    def _solve_annotations(self, annotations):
        for ann in annotations:
            ann.data[:, 0:2] *= self.output_stride
            if ann.data.shape[1]>3:
                ann.data[:, 3:5] *= self.output_stride
            if not 'butterfly' in self.model.head_names[0]:
                if ann.joint_scales is not None:
                    ann.joint_scales *= self.output_stride
            else:
                if ann.joint_scales_w is not None:
                    ann.joint_scales_w *= self.output_stride
                if ann.joint_scales_h is not None:
                    ann.joint_scales_h *= self.output_stride
