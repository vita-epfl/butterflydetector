...
def _scale(image, anns, meta, target_w, target_h, resample):
    ...
    for ann in anns:
        ...
        if 'class' in ann.keys():
            ann['class'][:, 0] = (ann['class'][:, 0]) * x_scale
            ann['class'][:, 1] = (ann['class'][:, 1]) * y_scale
        ann['bbox'][0] *= x_scale
        ann['bbox'][1] *= y_scale
        ann['bbox'][2] *= x_scale
        ann['bbox'][3] *= y_scale
...
class RescaleWH(Preprocess):
    def __init__(self, height, width, *, resample=PIL.Image.BICUBIC):
        self.width = width
        self.height = height
        self.resample = resample
    ...
    def __call__(self, image, anns, meta):
        ...
        return _scale(image, anns, meta, self.width, self.height, self.resample)

class ResizeAbsolute(Preprocess):
    def __init__(self, divisor, *, resample=PIL.Image.BICUBIC):
        self.divisor = divisor
        self.resample = resample

    def __call__(self, image, anns, meta):
        w, h = image.size
        target_w = -((-w) // self.divisor) * self.divisor
        target_h = -((-h) // self.divisor) * self.divisor
        return _scale(image, anns, meta, target_w, target_h, self.resample)
