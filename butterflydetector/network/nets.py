...
def factory_from_scratch(basename, headnames, *, pretrained=True):
    ...
    if 'hrnet' in basename:
        ...
        if 'det' in basename:
            detection = True
        return hrnet_factory_from_scratch(filename, headnames, detection)
...
def hrnet_factory_from_scratch(filename, headnames, detection=False):
...
def cli(parser):
    ...
    group.add_argument('--headnets', default=['butterfly'], nargs='+',
                       help='head networks')
